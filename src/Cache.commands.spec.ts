...
const mockLayer = (data) =>
    ({
        ...
        setItem: jest.fn(async function (key: string, value: StorageValue) {
            data[key] = value;
...
describe('Cache Commands', () => {
    ...
    describe('mget', () => {
        ...
        describe('multi-layer', function () {
            ...
            it('should return values from a the deepest layer ', async () => {
                ...
                expect(storageLayers[1].mset).toHaveBeenCalledWith([
                    ['foo', 'bar'],
                ]);
                //negative
    ...
    describe('mset', () => {
        ...
        it('should set multiple key-value pairs in all storageLayers', async () => {
            ...
            // mset
            expect(cache['storageLayers'][0].mset).toHaveBeenCalledWith([
                ['foo', 'hello'],
                ['bar', 'world'],
            ]);
            expect(cache['storageLayers'][1].mset).toHaveBeenCalledWith([
                ['foo', 'hello'],
                ['bar', 'world'],
            ]);
            // mget
            expect(
                await cache['storageLayers'][0].mget(['foo', 'bar'])
            ).toEqual(['hello', 'world']);
            expect(
                await cache['storageLayers'][1].mget(['foo', 'bar'])
            ).toEqual(['hello', 'world']);
        });
    });
    ...
    describe('sync', () => {
        ...
        it('should return the value for the provided key', async () => {
            ...
            // positive
            expect(await cache['storageLayers'][0].mget([key])).toEqual([
                value,
            ]);
        });
    });
    ...
    describe('set', () => {
        ...
        it('should set the value for the provided key in all storageLayers', async () => {
            ...
            const value = 'hello';
            await cache.set(key, value);
            expect(await cache['storageLayers'][2].mget([key])).toEqual([
                value,
            ]);
        });
    });
    ...
    describe('clear', () => {
        ...
        it('should call clear for all layers', async () => {
            ...
            expect(cache['storageLayers'][0].clear).toHaveBeenCalledWith(
                'token:',
                undefined
            );
            expect(cache['storageLayers'][1].clear).toHaveBeenCalledWith(
                'token:',
                undefined
            );
        });
